 ...
 */
@Slf4j
public final class TelegramAppender<E> extends UnsynchronizedAppenderBase<E> {

    private static final String SEND_MESSAGE_API_URL = 
    ...
    protected void sendTelegramMessage(E event) {
        String messageToSend = layout.doLayout(event);
        log.debug("sendTelegramMessage: {}", messageToSend);
        ...
        ResponseEntity<String> response =
            restTemplate.postForEntity(String.format(SEND_MESSAGE_API_URL, this.token), 
                                         new HttpEntity<>(body, this.headers),
                                         String.class);
